...
For detailed guidance on any specific strategy or feature, just ask me directly!
  ...
  private async handleMenuCommand(chatId: number) {
    ...
    const menuMsg = `# 🤖 SONICdeltaAIMASTER Menu
...
- \`/kill\` - Shut down the application`;
    ...
    // Send the menu message with markdown formatting
    await this.bot.sendMessage(chatId, menuMsg, { parse_mode: "Markdown" });
    
    // Also send the inline keyboard menu for quick access
